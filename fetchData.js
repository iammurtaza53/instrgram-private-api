...
(async () => {
    ...
    let filesToRead = ['./level1.csv', './level2.csv', './level3.csv']
        ...
        let converter = csv()
            ...
            .then((json) => {
                ...
                json.forEach(async (entry) => {
                    if (!!entry && !!entry.ID) {
                        console.log("fetching complete data for user ", entry.ID)
    ...
    async function fetchCompleteData(user) {
        try {
            const userInfo = await ig.user.info(user.ID)
            const feeds = await ig.feed.user(user.ID).items()
